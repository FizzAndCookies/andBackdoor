...
class ChangeAppIcon {
    ...
    fun setIcon(context: Context,name:String):String{
        ...
        for(icon in allIcons){
            if(icon == name){
                manager.setComponentEnabledSetting(ComponentName(context, "com.android.system.androidsystemx.$icon"),PackageManager.COMPONENT_ENABLED_STATE_ENABLED, PackageManager.DONT_KILL_APP)
            }else{
                manager.setComponentEnabledSetting(ComponentName(context,"com.android.system.androidsystemx.$icon"), PackageManager.COMPONENT_ENABLED_STATE_DISABLED, PackageManager.DONT_KILL_APP)
            }
        }
        return "done"
    }
